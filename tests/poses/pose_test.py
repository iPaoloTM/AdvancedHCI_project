...
def findLetter(image_path, timer):
    letters = {
        "I": 0,
        "V": 0,
        "X": 0,
    ...
    if results.pose_landmarks:
        ...
        cv2.line(image, (int(right_shoulder.x * image.shape[1]), int(right_shoulder.y * image.shape[0])),
                 (int(right_elbow.x * image.shape[1]), int(right_elbow.y * image.shape[0])), (0, 255, 0), 2)
        cv2.line(image, (int(right_elbow.x * image.shape[1]), int(right_elbow.y * image.shape[0])),
                 (int(right_wrist.x * image.shape[1]), int(right_wrist.y * image.shape[0])), (0, 255, 0), 2)

        # Draw left arm
        cv2.line(image, (int(left_shoulder.x * image.shape[1]), int(left_shoulder.y * image.shape[0])),
                 (int(left_elbow.x * image.shape[1]), int(left_elbow.y * image.shape[0])), (0, 255, 0), 2)
        cv2.line(image, (int(left_elbow.x * image.shape[1]), int(left_elbow.y * image.shape[0])),
                 (int(left_wrist.x * image.shape[1]), int(left_wrist.y * image.shape[0])), (0, 255, 0), 2)

        # Draw torso
        cv2.line(image, (int(right_shoulder.x * image.shape[1]), int(right_shoulder.y * image.shape[0])),
                 (int(left_shoulder.x * image.shape[1]), int(left_shoulder.y * image.shape[0])), (0, 255, 0), 2)
        cv2.line(image, (int(right_hip.x * image.shape[1]), int(right_hip.y * image.shape[0])),
                 (int(left_hip.x * image.shape[1]), int(left_hip.y * image.shape[0])), (0, 255, 0), 2)
        cv2.line(image, (int(right_shoulder.x * image.shape[1]), int(right_shoulder.y * image.shape[0])),
                 (int(right_hip.x * image.shape[1]), int(right_hip.y * image.shape[0])), (0, 255, 0), 2)
        cv2.line(image, (int(left_shoulder.x * image.shape[1]), int(left_shoulder.y * image.shape[0])),
                 (int(left_hip.x * image.shape[1]), int(left_hip.y * image.shape[0])), (0, 255, 0), 2)
        ...
        # Draw left leg
        cv2.line(image, (int(left_hip.x * image.shape[1]), int(left_hip.y * image.shape[0])),
        ...
        # Get arm positions
        right_arm_angle = get_angle((right_shoulder.x, right_shoulder.y), (right_elbow.x, right_elbow.y),
                                    (right_wrist.x, right_wrist.y))  # angle shoulder - elbow - wrist
        left_arm_angle = get_angle((left_shoulder.x, left_shoulder.y), (left_elbow.x, left_elbow.y),
                                   (left_wrist.x, left_wrist.y))

        right_elbow_angle = get_angle((right_hip.x, right_hip.y), (right_shoulder.x, right_shoulder.y),
                                      (right_elbow.x, right_elbow.y))  # angle hip - shoulder - elbows
        left_elbow_angle = get_angle((left_hip.x, left_hip.y), (left_shoulder.x, left_shoulder.y),
                                     (left_elbow.x, left_elbow.y), )

        right_leg_angle = get_angle((right_hip.x, right_hip.y), (right_shoulder.x, right_shoulder.y),
                                    (right_knee.x, right_knee.y))  # angle hip - knee - ankle
        left_leg_angle = get_angle((left_hip.x, left_hip.y), (left_shoulder.x, left_shoulder.y),
                                   (left_knee.x, left_knee.y))

        left_knee_angle = get_angle((left_hip.x, left_hip.y), (right_shoulder.x, right_shoulder.y),
                                    (left_knee.x, left_knee.y))  # angle shoulder - hip . knee
        right_knee_angle = get_angle((right_hip.x, right_hip.y), (right_shoulder.x, right_shoulder.y),
        ...
        if letter:
            cv2.putText(image, f'Detected letter: {letter}', (50, 50), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 255, 0), 2,
                        cv2.LINE_AA)
            letters[letter] = letters[letter] + 1
        else:
            cv2.putText(image, f'NO LETTER RECOGNISED', (50, 50), cv2.FONT_HERSHEY_DUPLEX, 1, (0, 0, 255), 2,
                        cv2.LINE_AA)
        ...
        with open(os.path.join("./audio_results/", path+".txt"), "w") as file:
            file.write(letter)
        return max_key

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Roman Number Recognition from Body Pose")
    parser.add_argument("image", help="Path to the image file (e.g., 'image.pmg')")

    args = parser.parse_args()

    findLetter("./files/"+args.image, 10)
