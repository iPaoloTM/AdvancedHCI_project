...
def recognize_speech_with_openai(path):
    try:

        audio_file = open(os.path.join("./files/", path+".mp3"), "rb")
...
def listen(mode, path):
    ...
    with microphone as source:
        ...
        try:
            ...
            transcription = recognize_speech_with_openai(path)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Roman Number Recognition from Audio")
    parser.add_argument("audio_path", help="Path to the audio file (e.g., 'audio.mp3')")

    args = parser.parse_args()

    listen('numbers', args.audio_path)
